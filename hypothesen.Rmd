...
datensatz <- transform(datensatz, stimuli=cut(datensatz$stimuli, breaks=c(-Inf, filter(datensatz$stimuli.group), Inf)))
...
```
#Das Verhalten wurde per Mediansplit in eine Gruppe mit hohem nachhaltigen Lebensstil und geringem nachhaltigen Lebensstil eingeteilt. Zur Hypothesenüberprüfung wurde nur die Gruppe mit hohem nachhaltigen Lebensstil betrachtet. Die Berechnung erfolgte mittels Pearson Moment Korrelation.
...
#Persönliche Einstellung 2:
Es gibt einen signifikanten Zusammenhang zwischen der Einstellung und dem Verhalten vor den Stimuli (r(57)=.66, p < .001)
...
Die lineare Regression zeigt, dass ein Modell it einem Prädiktor F(1,57)= 42.8, p<.001, adj.r²=419) signifikant wird. Das Modell klärt somit 41.9% mehr Varianz auf, als der Mittelwert alleine. Ob sich jemand nachhaltig Verhält, kann mit folgender Formel berechnet werden: 
